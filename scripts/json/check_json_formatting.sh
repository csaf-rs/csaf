...
done < <(find . \( -name 'target' -o -path './.git' -o -path './csaf' -o -path './ssvc' -o -path './csaf-rs/assets' -o -path './.vscode' -o -path './scripts' -o -path './type-generator/assets/rvisc' -o -path './type-generator/assets/language_specific_translations' -o -path './wasm/node_modules'  \) -prune -o -name '*.json' -not -name '*testcases_json_schema.json' -not -name '*json_schema.json' -print)
