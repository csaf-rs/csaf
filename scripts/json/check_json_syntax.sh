...
done < <(find . \( -name 'target' -o -path './.git' -o -path './csaf' -o -path './purl-spec' -o -path './ssvc' -o -path './csaf-rs/assets' -o -path './.vscode' -o -path './scripts' \) -prune -o -name '*.json' -not -name '*testcases.json' -not -name '*testcases_json_schema.json' -print)
