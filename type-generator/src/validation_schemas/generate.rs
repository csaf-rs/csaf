...
pub fn generate_url_file(entries: &[SchemaUrlEntry], target_folder: &str) -> Result<(), BuildError> {
    ...
    let mut file: syn::File = syn::parse2(items).expect("quote!-generated token stream should always be valid syntax");
...
pub fn generate_schema_file(entries: &[SchemaEntry], target_folder: &str) -> Result<(), BuildError> {
    ...
    let depth = Path::new(OUTPUT_PATH).parent().map_or(0, |p| p.components().count());
    ...
    let mut file: syn::File = syn::parse2(full).expect("quote!-generated token stream should always be valid syntax");
