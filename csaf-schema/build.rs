...
fn get_schemas() -> Vec<(&'static str, &'static str, Option<&'static dyn Fn(&mut Value)>)> {
    vec![
        (
...
fn get_testcases_schemas() -> Vec<(&'static str, &'static str, Option<&'static dyn Fn(&mut Value)>)> {
    vec![
        (
...
fn main() -> Result<(), BuildError> {
    ...
    let mut schema_configs = get_schemas();
    schema_configs.extend(get_testcases_schemas());
    ...
    for (input, output, schema_patch) in &schema_configs {
        build_from_schema(input, output, schema_patch)?;
    }
...
fn build_from_schema(input: &str, output: &str, schema_patch: &Option<&dyn Fn(&mut Value)>) -> Result<(), BuildError> {
