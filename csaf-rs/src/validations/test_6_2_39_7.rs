...
pub fn test_6_2_39_7_document_acknowledgments(doc: &impl CsafTrait) -> Result<(), Vec<TestFinding>> {
    ...
    if document.get_acknowledgments().is_none() {
        return Err(vec![create_missing_acknowledgments_warning()]);
    }

    Ok(())
}

fn create_missing_acknowledgments_warning() -> TestFinding {
...
mod tests {
    ...
    fn test_test_6_2_39_7() {
        let missing_acknowledgments = Err(vec![create_missing_acknowledgments_warning()]);

        // Case 01: document acknowledgments are missing
        TESTS_2_1.test_6_2_39_7.expect(ExpectedResults {
            case_01: missing_acknowledgments,
            // Case 11: document acknowledgments are present
            case_11: Ok(()),
        });
    }
}
