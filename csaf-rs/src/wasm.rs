...
pub fn validate_csaf(json_str: &str, preset_str: &str) -> Result<JsValue, JsValue> {
    // Parse the preset
    let preset = preset_str.parse::<ValidationPreset>()
        .map_err(|_| JsValue::from_str(&format!("Invalid preset: {}. Use 'basic', 'extended', or 'full'", preset_str)))?;
    ...
    let version = json_value
        ...
        .and_then(|v| v.as_str())
        .ok_or_else(|| JsValue::from_str("Could not detect CSAF version. Make sure the document has a 'document.csaf_version' field"))?;

    // Validate based on version
    let result = match version {
        "2.0" => validate_2_0(json_str, preset),
        "2.1" => validate_2_1(json_str, preset),
        _ => Err(format!("Unsupported CSAF version: {}. Supported versions: 2.0, 2.1", version)),
    };
