...
fn tests_for_preset(version: &CsafVersion, preset: &str) -> Result<Vec<String>, (StatusCode, Json<ErrorResponse>)> {
    match version {
        CsafVersion::X20 => Csaf2_0::tests_in_preset(preset).map_or_else(
            |e| Err(error_response(StatusCode::BAD_REQUEST, e.to_string())),
            |tests| Ok(tests.into_iter().map(String::from).collect()),
        ),
        CsafVersion::X21 => Csaf2_1::tests_in_preset(preset).map_or_else(
            |e| Err(error_response(StatusCode::BAD_REQUEST, e.to_string())),
            |tests| Ok(tests.into_iter().map(String::from).collect()),
        ),
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn returns_404_for_unknown_preset() {
